...
export function WhatsAppSender({ initialGuests, initialParty }: WhatsAppSenderProps) {
  ...
  const handleSendIndividual = async () => {
    ...
    try {
      ...
      if (selectedGuest) {
        ...
        if (!guest.whatsapp) {
          throw new Error("Este convidado não possui número de WhatsApp cadastrado")
        }
        ...
        result = await WhatsAppService.sendCustomMessage(guest.whatsapp, processedMessage)
      } else {
      ...
      setError(error.message || "Ocorreu um erro ao enviar a mensagem")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSendBulk = async () => {
    ...
    try {
      ...
      // Filtrar convidados com número de WhatsApp
      const guestsWithWhatsApp = guests.filter((guest) => guest.whatsapp)

      if (guestsWithWhatsApp.length === 0) {
        throw new Error("Nenhum convidado possui número de WhatsApp cadastrado")
      }

      // Confirmar envio em massa
      if (
        !window.confirm(
          `Você está prestes a enviar mensagens para ${guestsWithWhatsApp.length} convidados. Deseja continuar?`,
        )
      ) {
        setIsLoading(false)
        return
      ...
      for (const guest of guestsWithWhatsApp) {
        try {
          ...
          const result = await WhatsAppService.sendCustomMessage(guest.whatsapp!, processedMessage)
      ...
      setSuccess(`Mensagens enviadas: ${successCount} com sucesso, ${errorCount} com erro.`)
    } catch (error: any) {
      setError(error.message || "Ocorreu um erro ao enviar as mensagens")
    } finally {
      setIsLoading(false)
    }
  }
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          ...
          description: result.error || "Ocorreu um erro ao enviar a mensagem.",
          variant: "destructive",
        })
      }
    } catch (error) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Envie mensagens WhatsApp para convidados da festa</CardDescription>
      </CardHeader>
      <CardContent>
        {/* Seletor de festa */}
        ...
        {party && (
          <Tabs value={selectedTab} onValueChange={setSelectedTab}>
            ...
            <TabsContent value="individual" className="space-y-4">
                  ...
                  <SelectContent>
                    ...
                    {guests?.map((guest) => (
                      <SelectItem key={guest.id} value={guest.id} disabled={!guest.whatsapp}>
                        {guest.nome_principal} {!guest.whatsapp && "(Sem WhatsApp)"}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <InvitationPreview party={party} guest={selectedGuestObject} message={message} type="whatsapp" />
            ...
            <TabsContent value="bulk" className="space-y-4">
              <div className="p-4 bg-yellow-50 rounded-md border border-yellow-200">
                ...
                <p className="text-sm text-yellow-700">
                  Esta opção enviará mensagens para todos os convidados que possuem WhatsApp cadastrado (
              ...
              {guests.length > 0 && (
                <InvitationPreview party={party} guest={guests[0]} message={message} type="whatsapp" />
              )}
